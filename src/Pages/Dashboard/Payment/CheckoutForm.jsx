...
const CheckoutForm = ({ singleOrder }) => {
  ...
  useEffect(() => {
    fetch(
      `https://tools-manufactures.onrender.com/payment/create-payment-intent?uid=${auth?.currentUser?.uid}`,
      {
        method: "POST",
        headers: {
  ...
  const handleSubmit = async (event) => {
    ...
    if (intentError) {
      ...
      if (paymentIntent?.status === "succeeded") {
        ...
        fetch(
          `https://tools-manufactures.onrender.com/orders?uid=${auth?.currentUser?.uid}&&orderId=${singleOrder?._id}`,
          {
            method: "PATCH",
            headers: {
              authorization: `Bearer ${localStorage.getItem("accessToken")}`,
          ...
          .then((result) => {
            if (result.success) {
              ...
              fetch(
                `https://tools-manufactures.onrender.com/products?uid=${auth?.currentUser?.uid}&&productId=${singleOrder?.productInfo?.productId}`,
                {
                  method: "PATCH",
                  headers: {
